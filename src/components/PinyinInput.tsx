...
const PinyinInput: React.FC = () => {
  ...
  const onSubmit = (data: FormValues) => {
    const word = [...new Set(getValues().query)].join("");
    setWord(word);
  ...
  const queryToCharacter = () => {
    ...
    return [...new Set(chineseChars)].slice(0, 5);
  };
  ...
  return (
    <div className="relative w-full pb-2">
      ...
      >
        ...
        <button
          type="submit"
          className="px-2 bg-black text-white flex items-center justify-center cursor-pointer"
        >
          <Search size={18} />
        </button>
      </form>
      {errors.query?.type === "maxLength" && (
        <div className="absolute flex -bottom-5 left-0.5 gap-1 text-[10px] bg-black/10 flex-wrap">
